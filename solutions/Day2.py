...
def grab_policy(pol_pass: str):
    ...
    policy = space[1][0]
    password = space[2]
    return int(min), int(max), policy, password


def eval_password(min, max, pol, pswd):
    s = 0
    for c in pswd:
        if c == pol:
            s += 1
...
def eval_passwordp2(first, second, pol, pswd):
    s = 0
    a = pswd[first - 1] == pol
    b = pswd[second - 1] == pol
    return a != b  # a xor b
...
def day2(password_list):
    print('\033[1m' + "Password Philosophy" + '\033[0m')
    
    passwords = password_list.split("\n")
    # print(passwords)
    return day2p1(passwords), day2p2(passwords)


def day2p1(passwords):
    total = 0
    for pp in passwords:
        line = grab_policy(pp)
        valid = eval_password(line[0], line[1], line[2], line[3])
        # print(line, valid)
        if valid:
            total += 1
    return total
...
def day2p2(passwords):
    total = 0
    for pp in passwords:
        line = grab_policy(pp)
        valid = eval_passwordp2(line[0], line[1], line[2], line[3])
        # print(line, valid)
        if valid:
            total += 1
    return total
