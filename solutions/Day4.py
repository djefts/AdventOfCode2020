...
def hgt_check(hgt):
    unit = hgt[-2:]
    val = hgt[:-2]
    return ((unit == 'cm' and 150 <= int(val) <= 193)
            or (unit == 'in' and 59 <= int(val) <= 76))


def hcl_check(hcl):
    return re.fullmatch('^#[a-f0-9]{6}$', hcl)
...
def check_validity(passport):
    ...
    for field in list(fields.keys()):
        if field in passport:
            fields[field] = True
    return False not in fields.values()


def check_validityp2(passport):
    if not check_validity(passport):
        return False
    
    passport = passport.split(' ')
    # print(passport)
    methods = {'byr': byr_check,
               'iyr': iyr_check,
               'eyr': eyr_check,
               'hgt': hgt_check,
               'hcl': hcl_check,
    ...
    for field in passport:
        name = field.split(':')[0]
        val = field.split(':')[1]
        if not methods[name](val):
            return False
    return True


def day4(raw_input: str):
    print('\033[1m' + "Passport Processing" + '\033[0m')
    passports = raw_input.split('\n\n')
    passports = [passport.replace("\n", " ") for passport in passports]
    
    return day4p1(passports), day4p2(passports)


def day4p1(passports):
    valid = 0
    for passport in passports:
        # print(passport, "\n")
...
def day4p2(passports):
    valid = 0
    for passport in passports:
        if check_validityp2(passport):
            valid += 1
    return valid
